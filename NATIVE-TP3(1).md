...
This is now the cleaner primary checkpoint for documenting the native ModelOpt NVFP4 path.
...
A proper fixed-length multi-run benchmark should be used before publishing a definitive decode number.
...
However, **this is not a valid quality/performance benchmark**, because the generated output was corrupted.
...
## 3. local-inference-lab/GLM-5.3-Flash-NVFP4-4p67

This checkpoint is structurally different from the two pure NVFP4 checkpoints above.

vLLM detects ModelOpt metadata corresponding to:
...
### Why it fails with the current patch

The v6/v7 KDA workaround intentionally constructs the fused KDA projection as BF16:

```text
quant_config = None
```

because the pure NVFP4 checkpoints keep those KDA tensors in BF16.

This LIL checkpoint, however, carries quantization metadata for that fused KDA path.

Therefore its loader asks for:

```text
in_proj_qkvgfab.weight_scale
```

but the BF16 runtime parameter has no scale parameter.

This result does not mean the LIL checkpoint is bad; it means it needs a different KDA quantization/loading treatment.

---

## 4. brandonmusic/GLM-5.3-Flash-tr3-4bpw

This checkpoint was inspected but intentionally **not used for the newer ModelOpt NVFP4 line**.

Its routed expert weights use an EXL3/MCG packed representation.
...
with metadata origins such as:

```text
sealed_exl3_mcg_packed_choice
```

Many non-expert tensors are native copies:

```text
official_bf16_native_copy
```
...
A safetensors-index scan found:

```text
Total tensors: 150226
ModelOpt-style scale tensors: 0
```
...
So while the checkpoint is "mixed" in the broad sense that routed experts are quantized and other tensors stay BF16/FP32, it is **not `modelopt_mixed`**.

It belongs to the EXL3 execution path and was not considered a drop-in checkpoint for the native ModelOpt NVFP4 patches.

---

## 5. DFlash2 / MXFP8 variants

Also available during development:
...
These were not chosen as the primary checkpoint for the native NVFP4 branch because they represent different DFlash2 / MXFP8 execution and quantization paths.

They remain useful as related implementation references, especially because the base image itself originated from DFlash2/MXFP8 development work.
...
Because **both independent pure ModelOpt NVFP4 checkpoints load successfully but generate broken output**, the next debugging step should focus on separating **backend/kernel corruption** from **TP3 geometry/padding corruption**.
