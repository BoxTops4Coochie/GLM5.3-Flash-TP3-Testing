...
image, Compose settings and mounted launcher on September 15, 2026.
...
Built locally as `glm53-r34-tp3:native-dcp-20260913` (image ID
`sha256:49106c2466d7b…`) from `glm53-r34-tp3:llgemm-20260911b`, carrying the
R34 TP3 compatibility changes forward.

```bash
docker pull azallaza/glm53-r34-tp3:native-dcp-20260913
```

The previously published guide names `azallaza/glm53-r34-tp3:20260910`. That is
the older artifact and is superseded by the image above. The measurements in
this document were taken on the local build; the published image is the same
artifact under a registry name.
...
**The image alone is not enough.** The copy of the TP3 launcher baked into the
image is older than the one this configuration runs
(`sha256:17ea8f8a…` in the image versus `sha256:3839b565…` mounted). The image's
copy has no `REASONING_EFFORT` or `CLEAR_THINKING` handling at all — it
hardcodes `--default-chat-template-kwargs '{"reasoning_effort":"high",
"clear_thinking":true}'`. Running the image without the mount therefore
**ignores `REASONING_EFFORT` and serves `high`**, which the measurements below
show is 60% exact on the ledger test against 97% at `max`. It also still
accepts `ENABLE_PREFIX_CACHING=0` silently instead of rejecting it.

Download [serve-glm53-flash-tp3-r34.sh](serve-glm53-flash-tp3-r34.sh) and keep
it next to [compose.yaml](compose.yaml); the Compose file bind-mounts it over
the image copy. Verify it against `launcher_sha256` in
[baseline-receipt.json](baseline-receipt.json), which also records the exact
observed image, settings and source fingerprint.
